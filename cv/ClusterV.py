...
def run_clustering(_candidate_item, tree, is_tree_empty, _ref, _bed, _n_max_candidates=15, _min_af=0.05, _n_max_coverage=10000, _n_min_supports=50):
...
#     _run_command(cmd)
    ...
    cmd = 'cd %s; ls %s/%s.tagged.tag*.bam |  parallel -j 3 --joblog %s/run_all.log "time bash %s %s/{/} {/} %s %s %s/{/}_dir > %s/{/}_dir/run.log"' % \
    (_out_dir, _out_dir, _sample_idx, _out_dir, run_clair_path, _out_dir, _ref, _bed, _out_dir, _out_dir)
    _run_command(cmd)
...
#         print(_candidates_list)
        ...
        for _k in _candidates_list:
            if _k[5] == 0 and _flag_run_cluster == 1:
                ...
                rnt_item = run_clustering(_k, tree, is_tree_empty, _ref, _bed, _n_max_candidates=_n_max_candidates, _min_af=_min_af, _n_max_coverage=_n_max_coverage, _n_min_supports=_n_min_supports)
