...
max_iter = 100
dcov = 10000
# max_iter = number of read / 100
base_quality_cut_off = 5 # or int from 0-100
...
def generate_tensor(ctg_name, alignments, center, reference_sequence, reference_start_0_based, minimum_coverage, tensor_fp):
    ...
    #print("max_iter, %s" % (max_iter), file=sys.stderr)
...
def OutputAlnTensor(args):
    ...
    dcov = args.dcov
    max_iter = max(30, int(dcov/100))
    #dcov = 2500
    #print("max_iter, %s" % (max_iter), file=sys.stderr)
    #print("%s %s" % (max_iter, dcov), file=sys.stderr)
